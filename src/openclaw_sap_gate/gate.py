...
def _resolve_app_path(root: Path, app_id: str) -> Optional[Path]:
    """Return app directory under root, matching app_id case-insensitively."""
    exact = root / app_id
...
def _log_gap(app_id: str, reason: str) -> None:
    ...
    entry = {"ts": datetime.now(timezone.utc).isoformat(), "app_id": app_id,
             "event": "access_denied", "reason": reason}
    logger.warning("SAP gate denied: app_id=%s reason=%s", app_id, reason)
    with open(LOG_DIR / "gaps.jsonl", "a", encoding="utf-8") as f:
...
def _log_grant(app_id: str) -> None:
    ...
    with open(LOG_DIR / "grants.jsonl", "a", encoding="utf-8") as f:
        f.write(json.dumps(entry) + "\n")


def _verify_pgp(manifest_path: Path) -> tuple[bool, str]:
    """Verify detached GPG signature and confirm signer matches pinned fingerprint."""
...
def require_authorized(app_id: str) -> None:
    ...
    if not authorized(app_id):
        raise PermissionError(
            f"SAP gate denied: '{app_id}' — check SAFE folder, manifest, and .sig"
        )
